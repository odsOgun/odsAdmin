...
import { fetchSponsors, searchSponsors } from '../api/data'
...
const Sponsors = () => {
    ...
    useEffect(()=> {
        ...
        .then(res => {
            setLoading(true)
            // console.log(res);
            setUsers(res.data.sponsors)
            setFilteredUsers(res.data.sponsors);
  ...
  return (
    <div className='border-[1px] border-[#EAEAEA] rounded-[8px] p-[24px] bg-[#fff] min-h-[70vh]'>
        <div className='flex justify-between mb-[24px]'>
            <h2 className='font-medium text-[20px] text-[#121927] leading-[30px] tracking-[3%]'>SPONSORS ({count})</h2>
            ...
            <table className='w-[100%]'>
                ...
                <tbody>
                    {filteredUsers.map((user) => (<tr className='text-center text-[#101828] text-[14px] leading-[16.8px] tracking-[0] font-regular border-b-[#EAECF0] border-b-[1px]' key={user._id}>
                        <td className='py-[13px]' onClick={()=>navigate(`/sponsor-details?user=${user._id}`)}>{user.fullName}</td>
                        <td className='py-[13px]' onClick={()=>navigate(`/sponsor-details?user=${user._id}`)}>{user.email}</td>
                        <td className='py-[13px]' onClick={()=>navigate(`/sponsor-details?user=${user._id}`)}>{user.organisation}</td>
                        <td className='py-[13px]' onClick={()=>navigate(`/sponsor-details?user=${user._id}`)}>{user.phoneNumber}</td>
